...
def convex_tax_template(α_i_j, γ_i_j, t_k, τ_k):
    ...
    def get_U_i_j(v_j, i_idx, j_idx = None):
        if j_idx is None:
            v_j = v_j.unsqueeze(0)
            return α_i_j[i_idx] + (N_k.unsqueeze(0) + (1- τ_k.unsqueeze(0)) * (γ_i_j[i_idx] - v_j).unsqueeze(2)).amin(-1)
        else:
            return α_i_j[i_idx, j_idx] + (N_k + (1- τ_k) * (γ_i_j[i_idx, j_idx] - v_j).unsqueeze(1)).amin(-1)

    def get_V_i_j(u_i, j_idx, i_idx = None):
        if i_idx is None:
            u_i = u_i.unsqueeze(1)
            return γ_i_j[:, j_idx] + ((N_k.unsqueeze(0) + (α_i_j[:,j_idx] - u_i).unsqueeze(2))/(1- τ_k.unsqueeze(0))).amin(-1)
        else:
            return γ_i_j[i_idx,j_idx] + ((N_k + (α_i_j[i_idx,j_idx] - u_i).unsqueeze(1))/(1- τ_k)).amin(-1)
